...
/* This fixes a problem with 64-bit compilation */
#ifdef _UNIX
...
unsigned ULONGLONG randomseed;                     /* Current random number seed. */
...
"    -A  Assigns an additional floating-point attribute to each triangle\n");
  printf(
"        that identifies what segment-bounded region each triangle beULONGLONGs\n");
  printf(
...
"    Triangle calculates the intersection points for you and adds them to\n");
  printf(
"    the triangulation--as ULONGLONG as your machine's floating-point precision\n");
  printf(
...
"  row of evenly spaced, segment-connected vertices?  Have you simply\n");
  printf(
"  defined one ULONGLONG segment connecting the leftmost vertex to the rightmost\n"
);
  printf(
"  vertex, and a bunch of vertices lying aULONGLONG it?  This method occasionally\n"
);
  printf(
...

{
  ...
  printf("triangle x%lx with orientation %d:\n", (unsigned ULONGLONG) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    printf("    [0] = Outer space\n");
  } else {
    printf("    [0] = x%lx  %d\n", (unsigned ULONGLONG) printtri.tri,
           printtri.orient);
  }
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned ULONGLONG) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
    ...
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      printf("    [6] = x%lx  %d\n", (unsigned ULONGLONG) printsh.ss,
             printsh.ssorient);
    }
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (unsigned ULONGLONG) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (unsigned ULONGLONG) printvertex,
           printvertex[0], printvertex[1]);
}
...
/*  coordinate system; the destination of the triangle is one unit aULONGLONG the */
/*  xi axis; and the apex of the triangle is one unit aULONGLONG the eta axis.    */
...
#else /* not ANSI_DECLARATORS */
unsigned ULONGLONG randomnation(choices)
...
#else /* not ANSI_DECLARATORS */
ULONGLONG removeghosts(m, b, startghost)
struct mesh *m;
struct behavior *b;
...
#else /* not ANSI_DECLARATORS */
ULONGLONG incrementaldelaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#else /* not ANSI_DECLARATORS */
ULONGLONG sweeplinedelaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
ULONGLONG reconstruct(struct mesh *m, struct behavior *b, char *elefilename,
                 char *areafilename, char *polyfilename, FILE *polyfile)
#else /* not ANSI_DECLARATORS */
ULONGLONG reconstruct(m, b, elefilename, areafilename, polyfilename, polyfile)
struct mesh *m;
struct behavior *b;
char *elefilename;
char *areafilename;
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
      ...
      /* Ensure that the handle `badotri' does not represent the ULONGLONGest  */
...
#else /* not TRILIBRARY */
  FILE *outfile;
  ULONGLONG holenumber, regionnumber;
#endif /* not TRILIBRARY */
  struct osub subsegloop;
  vertex endpoint1, endpoint2;
